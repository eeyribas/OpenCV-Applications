...
ShowImgWithMatplotlib(cv2.cvtColor(th1, cv2.COLOR_GRAY2BGR), "Otsu's binarization (before applying a Gaussian filter)", 4)
ShowHistWithMatplotlibGray(hist2, "grayscale histogram", 5, 'm', ret2)
ShowImgWithMatplotlib(cv2.cvtColor(th2, cv2.COLOR_GRAY2BGR), "Otsu's binarization (after applying a Gaussian filter)", 6)
plt.show()